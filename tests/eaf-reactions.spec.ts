...
test.describe('EAF-style Reactions', () => {
  ...
  test('renders top-level agree/disagree counts from extendedScore on EAF host', async ({ page }) => {
    ...
    await expect(page.locator('.pr-header h1')).toContainText('EA Forum: Power Reader');
    ...
    const disagreeChip = page.locator('.pr-comment[data-id="c1"] .pr-reaction-chip[data-reaction-name="disagree"]');
  ...
  test('[PR-VOTE-06] renders agree/disagree chips with zero counts on EAF host', async ({ page }) => {
    await initPowerReader(page, {
      testMode: true,
      comments: [
        {
    ...
    const agreeChip = page.locator('.pr-comment[data-id="c-zero"] .pr-reaction-chip[data-reaction-name="agree"]');
    const disagreeChip = page.locator('.pr-comment[data-id="c-zero"] .pr-reaction-chip[data-reaction-name="disagree"]');

    await expect(agreeChip).toBeVisible();
    await expect(agreeChip.locator('.pr-reaction-count')).toHaveText('0');

    await expect(disagreeChip).toBeVisible();
    await expect(disagreeChip.locator('.pr-reaction-count')).toHaveText('0');
  });
  ...
  test('uses top-level payload format on EAF host even when votingSystem is missing', async ({ page }) => {
    await initPowerReader(page, {
      testMode: true,
      comments: [
        {
          _id: 'c-missing-system',
          postId: 'p1',
          extendedScore: {
            agree: 0,
            disagree: 0,
    ...
    const agreeChip = page.locator('.pr-comment[data-id="c-missing-system"] .pr-reaction-chip[data-reaction-name="agree"]');
    await agreeChip.click();

    const lastVars = await page.evaluate(() => (window as any).__LAST_MUTATION_VARS);
    expect(lastVars.extendedVote).toHaveProperty('agree', true);
    expect(lastVars.extendedVote).toHaveProperty('disagree', false);
  });
