...
- **AI Integration**: Send posts or comment threads to **Google AI Studio** (`g` / `Shift+G`) or **Arena.ai Max** (`m` / `Shift+M`) from both `/reader` and native LW/EAF post pages, with customizable prompt prefix for AI Studio; results are viewed directly in the opened provider tab.
