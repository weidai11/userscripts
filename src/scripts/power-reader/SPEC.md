...
- **[PR-UARCH-04] Background Sync**: After cache load, archive mode performs incremental sync for posts/comments newer than the stored watermark. Comments use cursor-based pagination with `timeField = "lastEditedAt"` for edit-aware syncing. Posts use offset-based pagination to avoid server-side cursor bugs (overlapping pages, missing records). For incremental sync, posts use `timeField = "modifiedAt"` + `after` as a server-side filter to avoid fetching the full archive; for full sync/resync the `after` filter is omitted.
- **[PR-UARCH-15] Offset-Based Post Pagination**: Posts use offset-based pagination (100 per batch) to avoid a server-side cursor bug that causes overlapping pages and permanently missing records. Only a page with an empty rows array ends the scan — short non-empty batches and single pages whose rows are all invalid (ghost/dropped rows) do NOT stop pagination, so dropped rows do not leave the rest of the archive un-fetched; two CONSECUTIVE all-invalid pages, however, mean the server is not advancing and stop the scan as a clamp (flagged as truncation so the watermark is preserved and a transient ghost-storm self-heals next sync). Pagination also stops after two consecutive pages containing only items already seen in earlier pages (server offset clamping/cycling); a repeated full page flags truncation, a repeated short page means the collection ended. The offset API enforces an upper limit of 2000 (`maxAllowedApiSkip`, rejects with "Exceeded maximum value for skip"); when the limit is reached after a full final batch, the loader stops gracefully, keeps the items fetched so far, keeps the pre-sync `lastSyncDate_posts` so a future sync retries the scan, and reports the truncation in the status line. A short final batch at the limit means the scan completed. A cap rejection on the FIRST request (offset 0) is a hard sync error: nothing has been fetched to keep, and rejecting offset 0 indicates server misbehavior worth surfacing. An archive whose size is an exact multiple of the batch size and ends precisely at the limit boundary (e.g. 2100 posts) is indistinguishable from truncation and is conservatively flagged as such; since the posts watermark stays at its pre-sync value, this costs a full re-scan on every subsequent sync plus a status note — never data loss. A repeating/cycling server likewise stops, restores the watermark, and reports a distinct truncation note (a transient clamp then self-heals on the next sync). The posts watermark is advanced only by the final save — never per-batch — so an interrupted scan re-scans instead of skipping the un-fetched tail. Completeness beyond the offset limit is unknowable; items past the limit may remain unreachable regardless of retries. This is acceptable — the most prolific users on LW have under 1500 posts, well within the limit. If the incremental query fails — on its first request, or with a validation/schema-shaped error on any later request (e.g., the server rejects the `timeField` argument or the `after`+`offset` combination — older deployments / EAF legacy API; verified live 2026-08-01 that EAF accepts the combination, so the fallback targets older deployments) — the loader MUST fall back to the full offset scan and apply the `modifiedAt > watermark` cutoff client-side; in that mode the cutoff is not monotonic in the scan order, so pagination continues until the scan is exhausted. Transient mid-scan failures (network/timeout) do NOT trigger the fallback; they retry via the normal retry path. A transient failure on the very first request may still trigger the fallback once (validation errors surface there; the full scan hits the same network, so the extra cost is bounded). An empty FIRST page from the incremental query is probed once — one modifiedAt-ordered request (the incremental query without the `after` filter, client-side cutoff) — before concluding "up to date": if newer items exist, the loader falls back to the full offset scan, so a server that accepts but misapplies the `after` filter cannot silently stall the posts watermark past new or recently edited posts. Comments still use adaptive cursor pagination with dynamic limit adjustment (50–1000 items, ~2.5s/batch target) and watermark-based cursor advancement.
